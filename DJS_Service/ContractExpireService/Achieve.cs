...
namespace ContractExpireService
{
    public class Achieve : IJob, IConfigClient
    {
        ...
        public void Execute(IJobExecutionContext context)
        {
            try
            {
                ...
                iLog.WriteLog(context.JobDetail.Key.Name + "合同到期提醒接口调用开始！", 0);
                ContractExpireService.Service.ContractExpireService server = new Service.ContractExpireService();
                server.SendStoreExpire();

                iLog.WriteLog(context.JobDetail.Key.Name + "合同到期提醒接口调用结束！", 0);
                DateTime timee = DateTime.Now;
                TimeSpan travelTime = timee - times;
                iLog.WriteLog(context.JobDetail.Key.Name + "合同到期提醒接口调用耗时:" + travelTime, 0);

            }
            catch (Exception ex)
            {
                iLog.WriteLog(context.JobDetail.Key.Name + "合同到期提醒接口调用失败！" + ex.Message, 1, true);
            }
        }
        ...
        public void SetConfig(string jobName)
        {
            ...
            if (!string.IsNullOrEmpty(configName))
            {
                ...
                iConfigMgr.SetConfig("ServiceCode", "S011");
                ...
                iConfigMgr.SetConfig("WarningDate", "30");
                iConfigMgr.SetConfig("EmailTitle", "大V合同到期预警"); //邮件的标题
        ...
        public void ExecuteT(string name)
        {
            try
            {
                ...
                iLog.WriteLog(name + "合同到期提醒接口调用开始！", 0);

                ContractExpireService.Service.ContractExpireService server = new Service.ContractExpireService();
                server.SendStoreExpire();

                iLog.WriteLog(name + "合同到期提醒接口调用结束！", 0);
                DateTime timee = DateTime.Now;
                iLog.WriteLog(name + "合同到期提醒接口调用耗时:" + (timee - times).Milliseconds, 0);
