                    ...
                    if (emailLists != null && emailLists.Count > 0)
                    {
                        foreach (Model.AMSServiceSetEmail itemT in emailLists)
                        {
                            try
                            {
                                ...
                                int storeId = 0;
                                if (Int32.TryParse(itemT.StoreID, out storeId))
                                {
                                    RentLoanService.SendRentLoanEmail(itemT.ID, (int)EnumUtility.EmailCategory.租金贷预警, storeId, (int)EnumUtility.EmialModel.Emial);
                                }
                            }
                            catch (Exception ex)
                            {
