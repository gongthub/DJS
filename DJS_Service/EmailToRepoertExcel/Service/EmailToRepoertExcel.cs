    ...
    public class EmailToRepoertExcel
    {
        ...
        public void DayRepExportExcel(string currentSheetName, string strTemplateName)
        {
            try
            {
                ...
                //List<string> storeIds = EmailUtility.GetServiceSetEmailStores(SERVICECODE);
                ...
                if (emailLists != null && emailLists.Count > 0)
                {
                    foreach (Model.AMSServiceSetEmail itemT in emailLists)
                    {
                        try
                        {
                            ...
                            iLog.WriteLog("门店ID：" + itemT.StoreID, 0);
                            iLog.WriteLog("收件人：" + itemT.TOPeople, 0);
                            iLog.WriteLog("抄送人：" + itemT.CCPeople, 0);
                            ...
                            {
                                //如果该店长管辖该门店，则生成一文件
                                if (itemT.StoreID.Split(',').Contains(s.StoreID.ToString()))
                                {
                                    ...
                                    excelWorksheet.Cells[1, 1] = s.StoreName + "租赁情况日报" + "(" + ConstUtility.EmailReportName + ")";
                                    ...
                                    foreach (var employee in sums)
                                    {
                                        ...
                                        foreach (SumRepoertTable dayRow in DayRepoerts)
                                        {
                                            ...
                                            switch (dayRow.SubjectType)
                                            {
                                                ...
                                                case 11: //其他收入【合同，门店】
                                                    ...
                                                    break;
                                                case 10: //成交均价（月）
                                                    excelWorksheet.Cells[22, columnIndex] = dayRow.SUMCoutOrAccount == 0 ? 0 : Math.Round((sumMoney / dayRow.SUMCoutOrAccount), 0, MidpointRounding.AwayFromZero);
                                                    break;
                                                case 30: //来电--58同城
                                                ...
                                                case 57: //累计租金贷放款金额
                                                    ...
                                                    break;
                                                case 51:  //36总房间数 37在住房间数   44在住率=在住房间数量/总房间数  
                                                    ...
                                                    excelWorksheet.Cells[45, columnIndex] = allRooms == 0 ? 0 : (yuDingCount + zaiZhuCount) / allRooms;
                                                    excelWorksheet.Cells[41, columnIndex] = (allRooms - zaiZhuCount - ydAndyyCount);
                                                    excelWorksheet.Cells[43, columnIndex] = allRooms - zaiZhuCount;
                                                    break;
                                                case 52: //预定房间数【预定，预约】
                                                    excelWorksheet.Cells[38, columnIndex] = dayRow.SUMCoutOrAccount;
                                                    break;
                                                case 55: //退租间数
                                                    excelWorksheet.Cells[39, columnIndex] = dayRow.SUMCoutOrAccount;
                                                    break;
                                                case 56: //续租间数
                                        ...
                                        excelWorksheet.Cells[42, columnIndex] = dayCount == 0 ? 0 : Math.Round(newSignAll / dayCount, 0, MidpointRounding.AwayFromZero);
                            ...
                            if (itemT.IsSum)
                            {
                                ...
                                //获取日报表汇总sql
                                string dayRepoertFileName1 = ConstUtility.ALLdayrepor;
                                ...
                                excelWorksheet1.Cells[1, 1] = "总部租赁情况日报" + "(" + ConstUtility.EmailReportName + ")";
                                ...
                                foreach (var employee in sums1)
                                {
                                    ...
                                    foreach (AllSumRepoertTable dayRow in DayRepoerts1)
                                    {
                                        ...
                                        switch (dayRow.SubjectType)
                                        {
                                            ...
                                            case 11: //其他收入【合同，门店】
                                                ...
                                                break;
                                            case 10: //成交均价（月）
                                                excelWorksheet1.Cells[22, columnIndex] = dayRow.SUMCoutOrAccount == 0 ? 0 : Math.Round((sumMoney / dayRow.SUMCoutOrAccount), 0, MidpointRounding.AwayFromZero);
                                                break;
                                            case 30: //来电--58同城
                                            ...
                                            case 51:  //36在住房间数   38在住率=在住房间数量/总房间数  39预定+在住（率）= （预定+在住）/运营房总间数 
                                                ...
                                                excelWorksheet1.Cells[45, columnIndex] = allRooms == 0 ? 0 : (yuDingCount + zaiZhuCount) / allRooms;
                                                excelWorksheet1.Cells[41, columnIndex] = (allRooms - zaiZhuCount - ydAndyyCount);
                                                excelWorksheet1.Cells[43, columnIndex] = allRooms - zaiZhuCount;
                                                break;
                                            case 52: //预定房间数【预定，预约】
                                                excelWorksheet1.Cells[38, columnIndex] = dayRow.SUMCoutOrAccount;
                                                break;
                                            case 55: //退租间数
                                                excelWorksheet1.Cells[39, columnIndex] = dayRow.SUMCoutOrAccount;
                                                break;
                                            case 56: //续租间数
                                    ...
                                    excelWorksheet1.Cells[42, columnIndex] = dayCount == 0 ? 0 : Math.Round(newSignAll / dayCount, 0, MidpointRounding.AwayFromZero);
                            ...
                            Store storeModel = ListStore.Find(m => m.StoreID.ToString() == itemT.StoreID);
                            ...
                            ZipFile(SaveExcelDirectory, DayDirectory, tempDD, sTo, sCC, storeName); //压缩文件
                        } 
                        catch (Exception ex)
                        {
                            iLog.WriteLog(ex.Message.ToString(), 1, true);
        ...
        public void ZipFile(string SaveExcelDirectory, string DayDirectory, string tempDD, List<string> sTo, List<string> sCC, string storeName)
        {
            ...
            if (ZipHelper.Zip(SaveExcelDirectory, ZipFileName)) //判断是否压缩成功
            {
                ...
                SendEmail(ZipFileName, sTo, sCC, storeName);
            }
        }
        ...
        public void SendEmail(string sfile, List<string> sTo, List<string> sCC, string storeName)
        {
            ...
            string title = ConstUtility.EmailReportName + ConstUtility.EmailTitle;
            string desc = ConstUtility.EmailDesc;
            ...
            EmailUtility.SendEmail(sTo, sCC, null, title, desc, sfile);
