    ...
    public class RentLoanSettlementService
    {
        ...
        public void SettlementAction()
        {
            ...
            try
            {
                ...
                {
                    List<RentLoanPool> tempRentLoanPools = rentLoanPools.Where(p => p.CertificateNo == rentLoanAudits[i].Renter.SSN && p.RenterName == rentLoanAudits[i].Renter.Name).ToList<RentLoanPool>();
                    ...
                    if (tempRentLoanPools.Count > 0)
                    {
                        ...
                        foreach (var PoolItem in tempRentLoanPools)
                        {
                            ...
                            List<Contract> tempContracts = contracts.Where(p => p.RenterID == rentLoanAudits[i].RenterID && p.Type == 1 && p.Status == 3).ToList<Contract>();
                            ...
                            foreach (var item in tempContracts)
                            {
                                ...
                                iLog.WriteLog("生成结算日志数据，合同号：" + item.ContractNo + " 租客：" + rentLoanHistoryPool.CertificateNo + " 结算金额：" + setmentLog.RepaymentAmount + " 还款日期：" + setmentLog.RepaymentTime,0);
