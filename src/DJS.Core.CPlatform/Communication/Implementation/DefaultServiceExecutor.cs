...
namespace DJS.Core.CPlatform.Communication.Implementation
{
    public class DefaultServiceExecutor : IServiceExecutor
    {
        ...
        public async Task ExecuteAsync(IMessageSender sender, TransportMessage message)
        {
            ...
            //{
            //执行本地代码。
            await LocalExecuteAsync(remoteInvokeMessage, resultMessage);
            //向客户端发送调用结果。
        ...
        private async Task LocalExecuteAsync(RemoteInvokeMessage remoteInvokeMessage, RemoteInvokeResultMessage resultMessage)
        {
            try
            {
