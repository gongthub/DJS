...
namespace DJS.Core.Scheduler.Configurations.Remote
{
    class RemoteConfigurationProvider : ConfigurationProvider
    {
        public RemoteConfigurationProvider(RemoteConfigurationSource source)
        {
            ...
            {
                Check.CheckCondition(() => source.ConfigurationKeyPrefix.Trim().StartsWith(":"), SchedulerResources.InvalidStartCharacter, "source.ConfigurationKeyPrefix", ":");
                Check.CheckCondition(() => source.ConfigurationKeyPrefix.Trim().EndsWith(":"), SchedulerResources.InvalidEndCharacter, "source.ConfigurationKeyPrefix",":");
            }
            Source = source;
        ...
        public override void Load()
        {
            ...
            try
            {
                ...
                {
                    throw new Exception(string.Format(SchedulerResources.HttpException, response.StatusCode, response.ReasonPhrase));
                }
            }
            catch (Exception)
            {
